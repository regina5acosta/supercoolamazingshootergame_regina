...
class ApiRepairman(object):
	...
	def __MakeApiBypassable(self, apiPath, platformKey):
		...
		allLines, HasUtf8Bom = BuildUtil.ImportCSharpFile(apiPath)

		if HasUtf8Bom:
			msg = '<{}> is encoded as UTF-8 BOM. Please convert to UTF-8 by commenting out this exception.'.format(apiPath)
			self.logger.error(msg)
			raise RuntimeError(msg)
